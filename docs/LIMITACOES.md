...
| HW-06 | **Beamforming do mic em HFP isola a voz de QUEM VESTE os óculos** — docs oficiais: "significantly reduces the volume of ambient sounds and other speakers. This is expected behavior" | A voz do PACIENTE chega atenuada — risco central para transcrição de consulta a dois | Testar no hackathon: (a) médico repete/parafraseia dados-chave (prática clínica comum), (b) mic do telefone sobre a mesa como captura de sala (capta os dois) com óculos para TTS/câmera, (c) híbrido. A escada L4 já implementa o caminho (b) | 🔴 (fonte oficial 19/08) |

## 2. DAT (Meta Wearables Device Access Toolkit 0.9.0)

| ID | Limitação | Impacto no projeto | Mitigação | Status |
|---|---|---|---|---|
| DAT-01 | **Microfone NÃO é API pública do DAT** — página oficial "Use device microphones and speakers": mic via **HFP pelo stack BT do sistema**; o exemplo Android oficial usa exatamente `MODE_IN_COMMUNICATION` + `setCommunicationDevice(TYPE_BLUETOOTH_SCO)` | Não existe "mic dos óculos" via SDK; o caminho é o BT do sistema | Nosso `AudioRouteManager` implementa o código idêntico ao da documentação oficial; ordering oficial respeitado (HFP estabilizado antes de stream.start) | 🟢 (idêntico ao exemplo oficial; confirmado 19/08) |
...
| AND-03 | **Áudio HFP é 8 kHz mono por especificação oficial** ("Audio is streamed as 8 kHz mono from the device to your app"); teste de comunidade mediu mSBC 16 kHz em hardware real — tratar 8 kHz como contrato | Qualidade do ASR degradada vs mic do celular | **Evidência medida (15/08, emulador, 8 kHz = spec oficial): degradação severa — "doutor"→"dor", "náusea" some.** Mitigações: modelo ASR maior; mic do telefone como captura de sala (v. HW-06); validar banda real no hackathon | 🟡 (spec medida; real pode ser melhor) |
...
| AND-07 | **Sem GitHub token na máquina de dev (15/08)** — artefatos Maven do DAT inacessíveis | Flavor `dat` do app não compila; só o flavor `sim` | Criar token `read:packages` + conta no Wearables Developer Center (ver DAT-08); no Developer Center, a permissão de **microfone exige justificativa** na aba Permissions (review interno da Meta) | 🔴 |
