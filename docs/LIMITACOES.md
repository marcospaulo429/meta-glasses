...
| AND-03 | **Áudio HFP/SCO é banda estreita (~8–16 kHz)** | Qualidade do ASR degradada vs mic do celular | Benchmark de WER com áudio HFP real; corpus PT-BR médico; se inaceitável, fallback: mic do celular como entrada | 🔴 |
...
| POL-04 | **Proposta do time (15/08) de vídeo contínuo como "gravação de segurança" médico-legal** — agrava frontalmente POL-01 (caso paradigmático de gravação contínua em local sensível vedado pela AUP) e falha o teste de necessidade da LGPD (art. 6º, III): finalidade atingível com áudio já consentido + fotos pontuais + log de auditoria | Se adotada, destrói o argumento de compliance do checkpoint "Privacidade e dados" e do critério ético (20 pts); risco de violação de plataforma | **Parecer LGPD ⛔ (15/08): não implementar.** Manter "câmera como evento" (MEMORY.md); proteção médico-legal via hash de integridade dos artefatos no log de auditoria (ver docs/LGPD.md §6). Limite paciente + 1 acompanhante aprovado como minimização | 🟡 |
